interface ProductCardProps {
  title: string;
  size: string;
  price: string;
  originalPrice: string;
  discount: string;
  image: string;
}

export default function ProductCard({ title, size, price, originalPrice, discount, image }: ProductCardProps) {
  return (
    <div className="flex flex-col items-center">
      {/* Product Image Container */}
      <div className="relative w-full max-w-[400px] h-[420px] mb-6">
        <img
          src={image}
          alt={title}
          className="w-full h-full object-cover rounded-lg"
        />
        {/* Size Badge */}
        <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-white/44 backdrop-blur-sm px-6 py-2 rounded-full">
          <span className="text-purple-badge text-lg font-bold">{size}</span>
        </div>
      </div>

      {/* Product Info */}
      <h3 className="text-xl font-semibold text-black text-center mb-2 leading-snug max-w-[242px]">
        {title}
      </h3>

      {/* Pricing */}
      <div className="flex items-center gap-3 mb-1">
        <span className="text-discount text-sm font-bold">{discount}</span>
        <span className="text-black text-[28px] font-bold">{price}</span>
      </div>
      <div className="mb-4">
        <span className="text-gray-text text-lg font-medium line-through">{originalPrice}</span>
      </div>

      {/* Action Buttons */}
      <div className="flex gap-3 w-full max-w-[500px]">
        <button className="flex-1 bg-[#E4E9FF] hover:bg-[#E4E9FF]/80 text-black px-4 py-4 rounded-md text-sm font-bold transition-colors">
          VIEW DETAILS
        </button>
        <button className="flex-1 bg-brand-yellow hover:bg-brand-yellow/90 text-brand-blue px-4 py-4 rounded-md text-sm font-bold transition-colors">
          ADD TO CART
        </button>
      </div>
    </div>
  );
}
